...
@push('style')
    <style>
        ...
        .small-mobile-text { font-size: 0.9rem; }
        .option-text { font-size: 1.1rem; }

        @media (min-width: 768px) {
            .small-mobile-text { font-size: 1.25rem; }
            .option-text { font-size: 1.4rem; }
            ...
            <div class="card border-0 rounded-4 rounded-md-5 overflow-hidden">
                <div class="card-body p-3 p-md-5">
                    @error('opsi_jawaban_id')
                        <div class="alert alert-danger alert-dismissible fade show rounded-4 border-0 shadow-sm mb-4 animate__animated animate__shakeX" role="alert">
                    ...
                    <div class="text-center mb-4">
                        <div class="image-frame p-2 p-md-3 bg-light rounded-4 border border-4 border-primary-subtle shadow-sm">
                    ...
                    <form method="POST" action="{{ route('siswa.ujian.submit') }}" id="formUjian">
                        ...
                        <div class="row g-3 g-md-4">
                            @foreach($soal->opsiJawaban as $key => $opsi)
                                ...
                                <div class="col-12 col-md-6">
                                    <input type="radio"
                                           name="opsi_jawaban_id"
                                           id="opsi_{{ $opsi->id }}"
                                           value="{{ $opsi->id }}"
                                           class="btn-check"
                                           @checked(optional($jawaban)->opsi_jawaban_id == $opsi->id)>
                                    
                                    <label class="btn-option d-flex align-items-center w-100 p-2 p-md-3 rounded-4 border-3 shadow-sm" 
                                           for="opsi_{{ $opsi->id }}">
                                        <div class="label-circle-mobile me-2 me-md-3" style="background-color: {{ $colors[$key] ?? '#5D87FF' }}">
                                            {{ $labels[$key] }}
                                        </div>
                        ...
                        <div class="nav-container d-flex justify-content-between align-items-center bg-white p-3 border-top shadow-lg">
                            ...
                            <button type="submit"
                                ...
                                <i class="ti {{ $nomor < $totalSoal ? 'ti-arrow-right' : 'ti-circle-check' }} ms-1"></i>
                            </button>
                        </div>
                    </form>
                </div>
