                ...
                <div class="col-9">
                    ...
                    <nav aria-label="breadcrumb" style="--bs-breadcrumb-divider: '/'">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item"><a class="text-muted text-decoration-none" href="{{ route('admin.siswa.index') }}">Data Siswa</a></li>
                            <li class="breadcrumb-item active text-primary" aria-current="page">{{ $siswa->nomor_ujian }}</li>
                        </ol>
                    </nav>
                </div>
                <div class="col-3">
    ...
    <div class="row">
        <div class="col-lg-4">
            <div class="card overflow-hidden shadow-sm">
                @php
                    $statusColor = !$siswa->hasilUjian()->exists() ? 'warning' : ($siswa->isLulus() ? 'success' : 'danger');
                    $statusText = !$siswa->hasilUjian()->exists() ? 'Belum Ujian' : ($siswa->isLulus() ? 'LULUS' : 'TIDAK LULUS');
                @endphp
                ...
                <div class="card-body text-center p-4">
                    ...
                    <h5 class="fw-bold mb-1">{{ $siswa->nama_lengkap }}</h5>
                    <p class="text-muted mb-3">{{ $siswa->nomor_ujian }}</p>
                ...
                <div class="border-top p-3 bg-light-subtle">
                    <div class="row text-center">
                        <div class="col-6 border-end">
                            ...
                            <p class="mb-0 text-muted">{{ $siswa->hasilUjian->jumlah_benar ?? '-' }}</p>
                        </div>
                        <div class="col-6">
                            <h6 class="mb-0 fw-bold">Daftar Ulang</h6>
                            <p class="mb-0 text-muted">{{ $siswa->pendaftaran ? 'Sudah' : 'Belum' }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="d-grid gap-2 mb-4">
                <a href="{{ route('admin.siswa.index') }}" class="btn btn-outline-primary fw-semibold">
                ...
                <div class="card-body">
                    @if ($siswa->hasilUjian)
                        <div class="d-flex align-items-center p-3 border rounded bg-light-subtle mb-3">
                            <div class="bg-white rounded p-3 shadow-sm text-center me-4" style="min-width: 100px;">
                                ...
                                <h2 class="mb-0 fw-bold text-primary">{{ $siswa->hasilUjian->jumlah_benar }}</h2>
                            </div>
                            <div>
                                <h6 class="fw-bold mb-1">Status Kelulusan</h6>
                                <p class="mb-0 text-muted small">
                                    {{ $siswa->hasilUjian->lulus 
                    ...
                    @else
                        <div class="text-center py-4 bg-light rounded border border-dashed">
                            ...
                            <p class="mb-0 text-muted">Belum ada data hasil ujian untuk siswa ini.</p>
                        </div>
                    @endif
                </div>
            </div>
                    ...
                    @if ($siswa->pendaftaran)
                        <div class="row g-4">
                            <div class="col-md-6">
                                ...
                                <h6 class="fw-semibold mb-0">{{ $siswa->pendaftaran->nama_orang_tua }}</h6>
                            </div>
                            <div class="col-md-6">
                                <label class="text-muted small d-block mb-1">Alamat Tinggal</label>
                                <h6 class="fw-semibold mb-0">{{ $siswa->pendaftaran->alamat }}</h6>
                            </div>
                            <div class="col-12 mt-4">
                                <div class="p-3 border rounded bg-primary-subtle">
                                    <h6 class="fw-bold text-primary mb-3">Lampiran Dokumen</h6>
                                    <div class="d-flex gap-2">
                                        <a href="{{ asset('storage/' . $siswa->pendaftaran->file_akta) }}" target="_blank" 
                                        ...
                                        <a href="{{ asset('storage/' . $siswa->pendaftaran->file_kk) }}" target="_blank" 
                                           class="btn btn-white shadow-sm border-0 flex-grow-1 d-flex align-items-center justify-content-center">
